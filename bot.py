            ...
            try:
                ...
                for task in build_queue:
                    ...
                    if 'location' in task and task['location'] > 0:
                        ...
                        if building and building.get('gid') == goal_gid and building.get('level', 0) >= goal_level:
                            is_complete = True
                            log.info(f"AGENT({self.village_name}): Sanitizing. Goal '{gid_name(goal_gid)} Lvl {goal_level} at Loc {task['location']}' is complete. Purging.")
                    elif not is_multi_instance(goal_gid):
                        if any(b.get('gid') == goal_gid and b.get('level', 0) >= goal_level for b in all_buildings):
                             is_complete = True
                             log.info(f"AGENT({self.village_name}): Sanitizing. Generic goal '{gid_name(goal_gid)} Lvl {goal_level}' is complete. Purging.")
                ...
                if goal_task.get('type') == 'resource_plan':
                    ...
                    if next_field_to_upgrade:
                        log.info(f"AGENT({self.village_name}): Resource plan: Upgrading {gid_name(next_field_to_upgrade['gid'])} at Loc {next_field_to_upgrade['id']} to Lvl {next_field_to_upgrade['level']+1}.")
                ...
                if not action_plan:
                    log.error(f"AGENT({self.village_name}): Could not determine action plan for goal {goal_task}. Village may be full or goal is already met. Skipping task for now.")
                    self.stop_event.wait(300); continue
                ...
                if build_result.get('status') == 'success':
                    ...
                    if goal_task.get('type') != 'resource_plan':
                        ...
                        save_config()
                    # --- EFFICIENCY FIX ---
                    # Instead of a long static wait, wait only a short time to prevent spam, then loop again.
